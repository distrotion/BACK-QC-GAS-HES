...
router.get('/CHECK-HGHMV001', async (req, res) => {

  return res.json(HGHMV001db['PO']);
});


router.post('/HGHMV001db', async (req, res) => {
  //-------------------------------------
  // console.log('--HGHMV001db--');
...
router.post('/GETINtoHGHMV001', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoHGHMV001--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGHMV001-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV001-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < HGHMV001db['ItemPickcode'].length; i++) {
    if (HGHMV001db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = HGHMV001db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    HGHMV001db['inspectionItem'] = ITEMSS;
    HGHMV001db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': HGHMV001db["PO"], 'CP': HGHMV001db["CP"], 'ITEMs': HGHMV001db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": HGHMV001db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            HGHMV001db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            HGHMV001db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            HGHMV001db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              HGHMV001db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              HGHMV001db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          HGHMV001db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          HGHMV001db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          HGHMV001db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          HGHMV001db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:16020/HGHMV001-feedback',
              { json: { "PO": HGHMV001db['PO'], "ITEMs": HGHMV001db['inspectionItem'] } },
...
router.post('/HGHMV001-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV001-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    HGHMV001db['GAPnameListdata'] = graph[0];//confirmdata
    HGHMV001db['GAP'] = HGHMV001db['GAPnameListdata'][`GT${HGHMV001db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/HGHMV001-preview', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV001-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGHMV001-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV001-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = HGHMV001db['preview'][0]

    if (HGHMV001db['RESULTFORMAT'] === 'Graph') {
      let pushdata = HGHMV001db['preview'][0]

      pushdata['V5'] = HGHMV001db['GAP'];
      pushdata['V1'] = `${HGHMV001db['confirmdata'].length + 1}:${pushdata['V1']}`;

      if(HGHMV001db['GAP'] !=''){

        HGHMV001db['confirmdata'].push(pushdata);
        HGHMV001db['preview'] = [];
        output = 'OK';
        HGHMV001db['GAP'] = HGHMV001db['GAPnameListdata'][`GT${HGHMV001db['confirmdata'].length + 1}`]
      }else{
    ...
    } else if (HGHMV001db['RESULTFORMAT'] === 'Number') {

      let pushdata = HGHMV001db['preview'][0]

      pushdata['V5'] = HGHMV001db['confirmdata'].length + 1
      pushdata['V1'] = `${HGHMV001db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/HGHMV001-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV001-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      HGHMV001db["PCSleft"] = `${parseInt(HGHMV001db["PCS"]) - oblist.length}`;
      if (HGHMV001db['RESULTFORMAT'] === 'Number' || HGHMV001db['RESULTFORMAT'] === 'Text' || HGHMV001db['RESULTFORMAT'] === 'Graph') {
        ...
        HGHMV001db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        HGHMV001db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(HGHMV001db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (HGHMV001db['GRAPHTYPE'] == 'CDE' ) {
              ...
              let core = 0;
              if (HGHMV001db['INTERSEC'] !== '') {
                core = parseFloat(HGHMV001db['INTERSEC'])
              } else {
              ...
              try {
                ...
                HGHMV001db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (HGHMV001db['GRAPHTYPE'] == 'CDT' ) {
              ...
              //
            } else if (HGHMV001db['GRAPHTYPE'] == 'CDT(S)' ) {
      ...
      HGHMV001db["ITEMleftUNIT"] = '';
      HGHMV001db["ITEMleftVALUE"] = '';
    }
...
router.post('/HGHMV001-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV001fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGHMV001-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV001fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/HGHMV001-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV001fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = HGHMV001db['confirmdata'].length
    if (all > 0) {
      HGHMV001db['confirmdata'].pop();
    }
...
router.post('/HGHMV001-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV001-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (HGHMV001db['RESULTFORMAT'] === 'Number' || HGHMV001db['RESULTFORMAT'] === 'Text') {

    HGHMV001db["value"] = [];
    for (i = 0; i < HGHMV001db['confirmdata'].length; i++) {
      HGHMV001db["value"].push({
        "PO1": HGHMV001db["inspectionItemNAME"],
        "PO2": HGHMV001db['confirmdata'][i]['V1'],
        "PO3": HGHMV001db['confirmdata'][i]['V2'],
        "PO4": HGHMV001db['confirmdata'][i]['V3'],
        "PO5": HGHMV001db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (HGHMV001db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < HGHMV001db["value"].length; i++) {
        mean01.push(parseFloat(HGHMV001db["value"][i]["PO3"]));
        mean02.push(parseFloat(HGHMV001db["value"][i]["PO5"]));
      }
      ...
      HGHMV001db["value"].push({
        "PO1": 'Mean',
        "PO2": HGHMV001db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": HGHMV001db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (HGHMV001db['RESULTFORMAT'] === 'OCR' || HGHMV001db['RESULTFORMAT'] === 'Picture') {

  } else if (HGHMV001db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < HGHMV001db['confirmdata'].length; i++) {
      HGHMV001db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": HGHMV001db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (HGHMV001db['RESULTFORMAT'] === 'Number' ||
    HGHMV001db['RESULTFORMAT'] === 'Text' ||
    HGHMV001db['RESULTFORMAT'] === 'OCR' ||
    HGHMV001db['RESULTFORMAT'] === 'Picture' || HGHMV001db['RESULTFORMAT'] === 'Graph') {
    request.post(
