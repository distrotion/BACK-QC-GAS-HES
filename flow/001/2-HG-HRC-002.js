...
router.get('/CHECK-HGHRC002', async (req, res) => {

  return res.json(HGHRC002db['PO']);
});


router.post('/HGHRC002db', async (req, res) => {
  //-------------------------------------
  // console.log('--HGHRC002db--');
...
router.post('/GETINtoHGHRC002', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoHGHRC002--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGHRC002-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--HGHRC002-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < HGHRC002db['ItemPickcode'].length; i++) {
    if (HGHRC002db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = HGHRC002db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    HGHRC002db['inspectionItem'] = ITEMSS;
    HGHRC002db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': HGHRC002db["PO"], 'CP': HGHRC002db["CP"], 'ITEMs': HGHRC002db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": HGHRC002db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            HGHRC002db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            HGHRC002db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            HGHRC002db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              HGHRC002db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              HGHRC002db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          HGHRC002db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          HGHRC002db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          HGHRC002db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          HGHRC002db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
...
router.post('/HGHRC002-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--HGHRC002-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    HGHRC002db['GAPnameListdata'] = graph[0];//confirmdata
    HGHRC002db['GAP'] = HGHRC002db['GAPnameListdata'][`GT${HGHRC002db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/HGHRC002-preview', async (req, res) => {
  //-------------------------------------
  console.log('--HGHRC002-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGHRC002-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--HGHRC002-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = HGHRC002db['preview'][0]

    if (HGHRC002db['RESULTFORMAT'] === 'Graph') {
      let pushdata = HGHRC002db['preview'][0]

      pushdata['V5'] = HGHRC002db['GAP'];
      pushdata['V1'] = `${HGHRC002db['confirmdata'].length + 1}:${pushdata['V1']}`;

      HGHRC002db['confirmdata'].push(pushdata);
      HGHRC002db['preview'] = [];
      output = 'OK';
      HGHRC002db['GAP'] = HGHRC002db['GAPnameListdata'][`GT${HGHRC002db['confirmdata'].length + 1}`]

    } else if (HGHRC002db['RESULTFORMAT'] === 'Number') {

      let pushdata = HGHRC002db['preview'][0]

      pushdata['V5'] = HGHRC002db['confirmdata'].length + 1
      pushdata['V1'] = `${HGHRC002db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/HGHRC002-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--HGHRC002-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      HGHRC002db["PCSleft"] = `${parseInt(HGHRC002db["PCS"]) - oblist.length}`;
      if (HGHRC002db['RESULTFORMAT'] === 'Number' || HGHRC002db['RESULTFORMAT'] === 'Text' || HGHRC002db['RESULTFORMAT'] === 'Graph') {
        ...
        HGHRC002db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        HGHRC002db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(HGHRC002db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (HGHRC002db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (HGHRC002db['INTERSEC'] !== '') {
                core = parseFloat(HGHRC002db['INTERSEC'])
              } else {
              ...
              try {
                ...
                HGHRC002db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (HGHRC002db['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
      ...
      HGHRC002db["ITEMleftUNIT"] = '';
      HGHRC002db["ITEMleftVALUE"] = '';
    }
...
router.post('/HGHRC002-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--HGHRC002fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGHRC002-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--HGHRC002fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/HGHRC002-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--HGHRC002fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = HGHRC002db['confirmdata'].length
    if (all > 0) {
      HGHRC002db['confirmdata'].pop();
    }
...
router.post('/HGHRC002-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--HGHRC002-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (HGHRC002db['RESULTFORMAT'] === 'Number' || HGHRC002db['RESULTFORMAT'] === 'Text') {

    HGHRC002db["value"] = [];
    for (i = 0; i < HGHRC002db['confirmdata'].length; i++) {
      HGHRC002db["value"].push({
        "PO1": HGHRC002db["inspectionItemNAME"],
        "PO2": HGHRC002db['confirmdata'][i]['V1'],
        "PO3": HGHRC002db['confirmdata'][i]['V2'],
        "PO4": HGHRC002db['confirmdata'][i]['V3'],
        "PO5": HGHRC002db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (HGHRC002db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < HGHRC002db["value"].length; i++) {
        mean01.push(parseFloat(HGHRC002db["value"][i]["PO3"]));
        mean02.push(parseFloat(HGHRC002db["value"][i]["PO5"]));
      }
      ...
      HGHRC002db["value"].push({
        "PO1": 'Mean',
        "PO2": HGHRC002db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": HGHRC002db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (HGHRC002db['RESULTFORMAT'] === 'OCR' || HGHRC002db['RESULTFORMAT'] === 'Picture') {

  } else if (HGHRC002db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < HGHRC002db['confirmdata'].length; i++) {
      HGHRC002db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": HGHRC002db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (HGHRC002db['RESULTFORMAT'] === 'Number' ||
    HGHRC002db['RESULTFORMAT'] === 'Text' ||
