...
router.get('/CHECK-HGHMV002', async (req, res) => {

  return res.json(HGHMV002db['PO']);
});


router.post('/HGHMV002db', async (req, res) => {
  //-------------------------------------
  // console.log('--HGHMV002db--');
...
router.post('/GETINtoHGHMV002', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoHGHMV002--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGHMV002-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV002-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < HGHMV002db['ItemPickcode'].length; i++) {
    if (HGHMV002db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = HGHMV002db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    HGHMV002db['inspectionItem'] = ITEMSS;
    HGHMV002db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': HGHMV002db["PO"], 'CP': HGHMV002db["CP"], 'ITEMs': HGHMV002db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": HGHMV002db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            HGHMV002db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            HGHMV002db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            HGHMV002db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              HGHMV002db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              HGHMV002db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          HGHMV002db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          HGHMV002db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          HGHMV002db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          HGHMV002db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:16020/HGHMV002-feedback',
              { json: { "PO": HGHMV002db['PO'], "ITEMs": HGHMV002db['inspectionItem'] } },
...
router.post('/HGHMV002-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV002-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    HGHMV002db['GAPnameListdata'] = graph[0];//confirmdata
    HGHMV002db['GAP'] = HGHMV002db['GAPnameListdata'][`GT${HGHMV002db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/HGHMV002-preview', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV002-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGHMV002-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV002-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = HGHMV002db['preview'][0]

    if (HGHMV002db['RESULTFORMAT'] === 'Graph') {
      let pushdata = HGHMV002db['preview'][0]

      pushdata['V5'] = HGHMV002db['GAP'];
      pushdata['V1'] = `${HGHMV002db['confirmdata'].length + 1}:${pushdata['V1']}`;

      if(HGHMV002db['GAP'] !=''){

        HGHMV002db['confirmdata'].push(pushdata);
        HGHMV002db['preview'] = [];
        output = 'OK';
        HGHMV002db['GAP'] = HGHMV002db['GAPnameListdata'][`GT${HGHMV002db['confirmdata'].length + 1}`]
      }else{
    ...
    } else if (HGHMV002db['RESULTFORMAT'] === 'Number') {

      let pushdata = HGHMV002db['preview'][0]

      pushdata['V5'] = HGHMV002db['confirmdata'].length + 1
      pushdata['V1'] = `${HGHMV002db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/HGHMV002-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV002-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      HGHMV002db["PCSleft"] = `${parseInt(HGHMV002db["PCS"]) - oblist.length}`;
      if (HGHMV002db['RESULTFORMAT'] === 'Number' || HGHMV002db['RESULTFORMAT'] === 'Text' || HGHMV002db['RESULTFORMAT'] === 'Graph') {
        ...
        HGHMV002db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        HGHMV002db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(HGHMV002db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (HGHMV002db['GRAPHTYPE'] == 'CDE' ) {
              ...
              let core = 0;
              if (HGHMV002db['INTERSEC'] !== '') {
                core = parseFloat(HGHMV002db['INTERSEC'])
              } else {
              ...
              try {
                ...
                HGHMV002db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (HGHMV002db['GRAPHTYPE'] == 'CDT' ) {
              ...
              //
            } else if (HGHMV002db['GRAPHTYPE'] == 'CDT(S)' ) {
      ...
      HGHMV002db["ITEMleftUNIT"] = '';
      HGHMV002db["ITEMleftVALUE"] = '';
    }
...
router.post('/HGHMV002-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV002fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGHMV002-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV002fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/HGHMV002-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV002fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = HGHMV002db['confirmdata'].length
    if (all > 0) {
      HGHMV002db['confirmdata'].pop();
    }
...
router.post('/HGHMV002-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV002-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (HGHMV002db['RESULTFORMAT'] === 'Number' || HGHMV002db['RESULTFORMAT'] === 'Text') {

    HGHMV002db["value"] = [];
    for (i = 0; i < HGHMV002db['confirmdata'].length; i++) {
      HGHMV002db["value"].push({
        "PO1": HGHMV002db["inspectionItemNAME"],
        "PO2": HGHMV002db['confirmdata'][i]['V1'],
        "PO3": HGHMV002db['confirmdata'][i]['V2'],
        "PO4": HGHMV002db['confirmdata'][i]['V3'],
        "PO5": HGHMV002db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (HGHMV002db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < HGHMV002db["value"].length; i++) {
        mean01.push(parseFloat(HGHMV002db["value"][i]["PO3"]));
        mean02.push(parseFloat(HGHMV002db["value"][i]["PO5"]));
      }
      ...
      HGHMV002db["value"].push({
        "PO1": 'Mean',
        "PO2": HGHMV002db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": HGHMV002db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (HGHMV002db['RESULTFORMAT'] === 'OCR' || HGHMV002db['RESULTFORMAT'] === 'Picture') {

  } else if (HGHMV002db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < HGHMV002db['confirmdata'].length; i++) {
      HGHMV002db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": HGHMV002db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (HGHMV002db['RESULTFORMAT'] === 'Number' ||
    HGHMV002db['RESULTFORMAT'] === 'Text' ||
    HGHMV002db['RESULTFORMAT'] === 'OCR' ||
    HGHMV002db['RESULTFORMAT'] === 'Picture' || HGHMV002db['RESULTFORMAT'] === 'Graph') {
    request.post(
