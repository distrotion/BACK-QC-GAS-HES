...
router.get('/CHECK-HGHMV003', async (req, res) => {

  return res.json(HGHMV003db['PO']);
});


router.post('/HGHMV003db', async (req, res) => {
  //-------------------------------------
  // console.log('--HGHMV003db--');
...
router.post('/GETINtoHGHMV003', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoHGHMV003--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGHMV003-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV003-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < HGHMV003db['ItemPickcode'].length; i++) {
    if (HGHMV003db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = HGHMV003db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    HGHMV003db['inspectionItem'] = ITEMSS;
    HGHMV003db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': HGHMV003db["PO"], 'CP': HGHMV003db["CP"], 'ITEMs': HGHMV003db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": HGHMV003db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            HGHMV003db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            HGHMV003db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            HGHMV003db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              HGHMV003db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              HGHMV003db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          HGHMV003db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          HGHMV003db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          HGHMV003db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          HGHMV003db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:16020/HGHMV003-feedback',
              { json: { "PO": HGHMV003db['PO'], "ITEMs": HGHMV003db['inspectionItem'] } },
...
router.post('/HGHMV003-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV003-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    HGHMV003db['GAPnameListdata'] = graph[0];//confirmdata
    HGHMV003db['GAP'] = HGHMV003db['GAPnameListdata'][`GT${HGHMV003db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/HGHMV003-preview', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV003-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGHMV003-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV003-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = HGHMV003db['preview'][0]

    if (HGHMV003db['RESULTFORMAT'] === 'Graph') {
      let pushdata = HGHMV003db['preview'][0]

      pushdata['V5'] = HGHMV003db['GAP'];
      pushdata['V1'] = `${HGHMV003db['confirmdata'].length + 1}:${pushdata['V1']}`;

      if(HGHMV003db['GAP'] !=''){

        HGHMV003db['confirmdata'].push(pushdata);
        HGHMV003db['preview'] = [];
        output = 'OK';
        HGHMV003db['GAP'] = HGHMV003db['GAPnameListdata'][`GT${HGHMV003db['confirmdata'].length + 1}`]
      }else{
    ...
    } else if (HGHMV003db['RESULTFORMAT'] === 'Number') {

      let pushdata = HGHMV003db['preview'][0]

      pushdata['V5'] = HGHMV003db['confirmdata'].length + 1
      pushdata['V1'] = `${HGHMV003db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/HGHMV003-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV003-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      HGHMV003db["PCSleft"] = `${parseInt(HGHMV003db["PCS"]) - oblist.length}`;
      if (HGHMV003db['RESULTFORMAT'] === 'Number' || HGHMV003db['RESULTFORMAT'] === 'Text' || HGHMV003db['RESULTFORMAT'] === 'Graph') {
        ...
        HGHMV003db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        HGHMV003db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(HGHMV003db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (HGHMV003db['GRAPHTYPE'] == 'CDE' ) {
              ...
              let core = 0;
              if (HGHMV003db['INTERSEC'] !== '') {
                core = parseFloat(HGHMV003db['INTERSEC'])
              } else {
              ...
              try {
                ...
                HGHMV003db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (HGHMV003db['GRAPHTYPE'] == 'CDT' ) {
              ...
              //
            } else if (HGHMV003db['GRAPHTYPE'] == 'CDT(S)' ) {
      ...
      HGHMV003db["ITEMleftUNIT"] = '';
      HGHMV003db["ITEMleftVALUE"] = '';
    }
...
router.post('/HGHMV003-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV003fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGHMV003-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV003fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/HGHMV003-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV003fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = HGHMV003db['confirmdata'].length
    if (all > 0) {
      HGHMV003db['confirmdata'].pop();
    }
...
router.post('/HGHMV003-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--HGHMV003-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (HGHMV003db['RESULTFORMAT'] === 'Number' || HGHMV003db['RESULTFORMAT'] === 'Text') {

    HGHMV003db["value"] = [];
    for (i = 0; i < HGHMV003db['confirmdata'].length; i++) {
      HGHMV003db["value"].push({
        "PO1": HGHMV003db["inspectionItemNAME"],
        "PO2": HGHMV003db['confirmdata'][i]['V1'],
        "PO3": HGHMV003db['confirmdata'][i]['V2'],
        "PO4": HGHMV003db['confirmdata'][i]['V3'],
        "PO5": HGHMV003db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (HGHMV003db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < HGHMV003db["value"].length; i++) {
        mean01.push(parseFloat(HGHMV003db["value"][i]["PO3"]));
        mean02.push(parseFloat(HGHMV003db["value"][i]["PO5"]));
      }
      ...
      HGHMV003db["value"].push({
        "PO1": 'Mean',
        "PO2": HGHMV003db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": HGHMV003db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (HGHMV003db['RESULTFORMAT'] === 'OCR' || HGHMV003db['RESULTFORMAT'] === 'Picture') {

  } else if (HGHMV003db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < HGHMV003db['confirmdata'].length; i++) {
      HGHMV003db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": HGHMV003db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (HGHMV003db['RESULTFORMAT'] === 'Number' ||
    HGHMV003db['RESULTFORMAT'] === 'Text' ||
    HGHMV003db['RESULTFORMAT'] === 'OCR' ||
    HGHMV003db['RESULTFORMAT'] === 'Picture' || HGHMV003db['RESULTFORMAT'] === 'Graph') {
    request.post(
