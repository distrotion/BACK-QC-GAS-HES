...
router.get('/CHECK-HGVHT001', async (req, res) => {

  return res.json(HGVHT001db['PO']);
});


router.post('/HGVHT001db', async (req, res) => {
  //-------------------------------------
  console.log('--HGVHT001db--');
...
router.post('/GETINtoHGVHT001', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoHGVHT001--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGVHT001-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--HGVHT001-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < HGVHT001db['ItemPickcode'].length; i++) {
    if (HGVHT001db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = HGVHT001db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    HGVHT001db['inspectionItem'] = ITEMSS;
    HGVHT001db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': HGVHT001db["PO"], 'CP': HGVHT001db["CP"], 'ITEMs': HGVHT001db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": HGVHT001db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            HGVHT001db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            HGVHT001db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            HGVHT001db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              HGVHT001db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              HGVHT001db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          HGVHT001db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          HGVHT001db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          HGVHT001db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          HGVHT001db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
...
router.post('/HGVHT001-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--HGVHT001-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    HGVHT001db['GAPnameListdata'] = graph[0];//confirmdata
    HGVHT001db['GAP'] = HGVHT001db['GAPnameListdata'][`GT${HGVHT001db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/HGVHT001-preview', async (req, res) => {
  //-------------------------------------
  console.log('--HGVHT001-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGVHT001-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--HGVHT001-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = HGVHT001db['preview'][0]

    if (HGVHT001db['RESULTFORMAT'] === 'Graph') {
      let pushdata = HGVHT001db['preview'][0]

      pushdata['V5'] = HGVHT001db['GAP'];
      pushdata['V1'] = `${HGVHT001db['confirmdata'].length + 1}:${pushdata['V1']}`;

      if(HGVHT001db['GAP'] !=''){

        HGVHT001db['confirmdata'].push(pushdata);
        HGVHT001db['preview'] = [];
        output = 'OK';
        HGVHT001db['GAP'] = HGVHT001db['GAPnameListdata'][`GT${HGVHT001db['confirmdata'].length + 1}`]
      }else{
    ...
    } else if (HGVHT001db['RESULTFORMAT'] === 'Number') {

      let pushdata = HGVHT001db['preview'][0]

      pushdata['V5'] = HGVHT001db['confirmdata'].length + 1
      pushdata['V1'] = `${HGVHT001db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/HGVHT001-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--HGVHT001-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      HGVHT001db["PCSleft"] = `${parseInt(HGVHT001db["PCS"]) - oblist.length}`;
      if (HGVHT001db['RESULTFORMAT'] === 'Number' || HGVHT001db['RESULTFORMAT'] === 'Text' || HGVHT001db['RESULTFORMAT'] === 'Graph') {
        ...
        HGVHT001db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        HGVHT001db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(HGVHT001db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (HGVHT001db['GRAPHTYPE'] == 'CDE' ) {
              ...
              let core = 0;
              if (HGVHT001db['INTERSEC'] !== '') {
                core = parseFloat(HGVHT001db['INTERSEC'])
              } else {
              ...
              try {
                ...
                HGVHT001db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (HGVHT001db['GRAPHTYPE'] == 'CDT' ) {
              ...
              //
            } else if (HGVHT001db['GRAPHTYPE'] == 'CDT(S)' ) {
      ...
      HGVHT001db["ITEMleftUNIT"] = '';
      HGVHT001db["ITEMleftVALUE"] = '';
    }
...
router.post('/HGVHT001-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--HGVHT001fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HGVHT001-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--HGVHT001fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/HGVHT001-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--HGVHT001fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = HGVHT001db['confirmdata'].length
    if (all > 0) {
      HGVHT001db['confirmdata'].pop();
    }
...
router.post('/HGVHT001-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--HGVHT001-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (HGVHT001db['RESULTFORMAT'] === 'Number' || HGVHT001db['RESULTFORMAT'] === 'Text') {

    HGVHT001db["value"] = [];
    for (i = 0; i < HGVHT001db['confirmdata'].length; i++) {
      HGVHT001db["value"].push({
        "PO1": HGVHT001db["inspectionItemNAME"],
        "PO2": HGVHT001db['confirmdata'][i]['V1'],
        "PO3": HGVHT001db['confirmdata'][i]['V2'],
        "PO4": HGVHT001db['confirmdata'][i]['V3'],
        "PO5": HGVHT001db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (HGVHT001db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < HGVHT001db["value"].length; i++) {
        mean01.push(parseFloat(HGVHT001db["value"][i]["PO3"]));
        mean02.push(parseFloat(HGVHT001db["value"][i]["PO5"]));
      }
      ...
      HGVHT001db["value"].push({
        "PO1": 'Mean',
        "PO2": HGVHT001db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": HGVHT001db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (HGVHT001db['RESULTFORMAT'] === 'OCR' || HGVHT001db['RESULTFORMAT'] === 'Picture') {

  } else if (HGVHT001db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < HGVHT001db['confirmdata'].length; i++) {
      HGVHT001db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": HGVHT001db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (HGVHT001db['RESULTFORMAT'] === 'Number' ||
    HGVHT001db['RESULTFORMAT'] === 'Text' ||
